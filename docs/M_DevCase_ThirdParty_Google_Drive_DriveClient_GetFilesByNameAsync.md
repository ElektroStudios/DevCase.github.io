# DriveClient.GetFilesByNameAsync Method 
 

Asynchronously gets the files stored in the current user account that matches the specified name.
...
**C#**<br />
``` C#
public Task<List<File>> GetFilesByNameAsync(
	string displayName
)
```
...
**VB**<br />
``` VB
Public Function GetFilesByNameAsync ( 
...
returnValue = instance.GetFilesByNameAsync(displayName)
```

**C++**<br />
``` C++
public:
Task<List<File^>^>^ GetFilesByNameAsync(
	String^ displayName
)
```
...
#### Parameters
&nbsp;<dl><dt>displayName</dt><dd>Type: System.String<br />\[Missing <param name="displayName"/> documentation for "M:DevCase.ThirdParty.Google.Drive.DriveClient.GetFilesByNameAsync(System.String)"\]</dd></dl>
...
Dim files As List(Of Google.Apis.Drive.v3.Data.File) = Await client.GetFilesAsyncByDisplayName("File Name")
