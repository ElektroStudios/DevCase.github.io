# AssemblyExtensions.IsCOMVisible Method 
 

Gets a value that determine whether the source Assembly is marked as COM visible.
...
public static bool IsCOMVisible(
...
<ExtensionAttribute>
<EditorBrowsableAttribute(EditorBrowsableState.Always)>
Public Shared Function IsCOMVisible ( 
	sender As Assembly
) As Boolean
```
...
returnValue = sender.IsCOMVisible()
```
...
#### Return Value
Type: Boolean<br />`true` (`True` in Visual Basic) if the source Assembly is marked as COM visible; otherwise, `false` (`False` in Visual Basic).
...
Dim isComVisible As Boolean = IsCOMVisible(asm)

Console.WriteLine(isComVisible)
```
