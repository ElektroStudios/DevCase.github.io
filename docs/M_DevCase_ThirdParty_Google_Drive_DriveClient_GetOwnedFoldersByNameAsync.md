# DriveClient.GetOwnedFoldersByNameAsync Method 
 

Asynchronously gets the folders stored in the current user account that are owned by him and matches the specified name.
...
**C#**<br />
``` C#
public Task<List<File>> GetOwnedFoldersByNameAsync(
	string displayName
)
```
...
**VB**<br />
``` VB
Public Function GetOwnedFoldersByNameAsync ( 
...
returnValue = instance.GetOwnedFoldersByNameAsync(displayName)
```

**C++**<br />
``` C++
public:
Task<List<File^>^>^ GetOwnedFoldersByNameAsync(
	String^ displayName
)
```
...
**F#**<br />
``` F#
member GetOwnedFoldersByNameAsync : 
...
#### Parameters
&nbsp;<dl><dt>displayName</dt><dd>Type: System.String<br />\[Missing <param name="displayName"/> documentation for "M:DevCase.ThirdParty.Google.Drive.DriveClient.GetOwnedFoldersByNameAsync(System.String)"\]</dd></dl>
...
Dim folders As List(Of Google.Apis.Drive.v3.Data.File) = Await client.GetOwnedFoldersByNameAsync("Folder Name")
